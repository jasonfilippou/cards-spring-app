...
/**
 * Controller for querying the API for {@link CardDto} instances. Supports operations for POST, GET (id and aggreggate),
...
public class CardController {
    ...
    public ResponseEntity<EntityModel<CardDto>> postCard(@RequestBody @Valid CardDto cardDto){
        return new ResponseEntity<>(assembler.toModel(cardService.storeCard(cardDto)), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<CollectionModel<EntityModel<CardDto>>> aggregateGetCards(
        ...
        List<String> cardFieldNames = Arrays.stream(CardDto.class.getDeclaredFields()).
                map(Field::getName).toList();
        if(!cardFieldNames.contains(sortByField)){
            throw new InvalidSortByFieldException(sortByField, cardFieldNames);
        }
        return ResponseEntity.ok(assembler.toCollectionModel(cardService.getAllCardsByFilter(
